...
export const add = new Command()
  ...
  .action(async (components, opts) => {
    try {
      ...
      if (!config) {
        consola.warn(
          `Configuration is missing. Please run ${colors.green('init')} to create a .keen.json file.`
        )
      ...
      for (const item of payload) {
        ...
        if (!existsSync(componentDir)) await fs.mkdir(componentDir, { recursive: true })

        const files = item.files.map((file) => ({
          ...file,
          path: path.resolve(targetDir, item.name, file.name)
        }))

        // We need to write original files to disk if we're not using TypeScript.
        // Rewrite or delete added files after transformed
        if (!config.typescript) {
          for (const file of files) await fs.writeFile(file.path, file.content)
        }
