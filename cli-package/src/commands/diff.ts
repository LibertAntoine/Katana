...
import { type Change, diffLines } from '@/src/commands/diff'
import { z } from 'zod'
    ...
    try {
      ...
      if (!options.component) {
        ...
        for (const component of componentsWithUpdates) {
          ...
          for (const change of component.changes) consola.info(`  - ${change.filePath}`)
        }
      ...
      if (!component) {
        consola.error(`The component ${colors.green(options.component)} does not exist.`)
        process.exit(1)
      }
...
function printDiff(diff: Change[]) {
  diff.forEach((part) => {
    if (part) {
      if (part.added) return process.stdout.write(colors.green(part.value))

      if (part.removed) return process.stdout.write(colors.red(part.value))
