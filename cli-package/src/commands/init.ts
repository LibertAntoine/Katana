...
const PROJECT_DEPENDENCIES = {
  base: ['tailwindcss-animate', 'class-variance-authority', 'clsx', 'tailwind-merge', 'radix-vue'],
...
export async function promptForConfig(
  ...
  const options = await prompts([
    ...
    },
    {
      ...
      initial: (prev, values) =>
        ...
        TAILWIND_CSS_PATH[values.framework as 'vite' | 'nuxt' | 'laravel' | 'astro']
    },
    {
      type: 'toggle',
  ...
  if (!skip) {
    const { proceed } = await prompts({
      type: 'confirm',
      name: 'proceed',
      message: `Write configuration to ${highlight('.keen.json')}. Proceed?`,
      initial: true
    })
  ...
  // Write to file.
  consola.log('')
  const spinner = ora('Writing .keen.json...').start()
  const targetPath = path.resolve(cwd, '.keen.json')
...
export async function runInit(cwd: string, config: Config) {
  ...
  for (const [key, resolvedPath] of Object.entries(config.resolvedPaths)) {
    ...
    // If the utils alias is set to something like "@/utils",
  ...
  await fs.writeFile(
    ...
    transformCJSToESM(
      ...
      config.tailwind.cssVariables
        ...
        : template(templates.TAILWIND_CONFIG)({
            extension,
            framework: config.framework,
            prefix: config.tailwind.prefix
          })
  ...
  const deps = PROJECT_DEPENDENCIES.base
    .concat(config.style === 'new-york' ? ['@radix-icons/vue'] : ['lucide-vue-next'])
    .filter(Boolean)
