...
const DEPENDENCIES_WITH_TAGS = new Map<string, string>([['v-calendar', 'v-calendar@next']])
...
export async function buildRegistry() {
  const ui_path = resolve('./src/components/default/ui')
  const uiRegistry = await crawlUI(ui_path)

  // const example_path = resolve('./src/registry/default/example')
  // const exampleRegistry = await crawlDirectory(example_path, 'example')

  // const block_path = resolve('./src/registry/default/block')
...
async function getDependencies(filename: string) {
  ...
  const populateDeps = (source: string) => {
    ...
    if (peerDeps !== undefined) {
      if (taggedDeps !== undefined) dependencies.add(taggedDeps)
      else dependencies.add(source)
