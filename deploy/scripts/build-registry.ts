...
import { colorMapping, colors } from '../../src/components/colors'
import { registrySchema } from '../../src/components/schema'
import { styles } from '../../src/components/styles'
import { themes } from '../../src/components/themes'
import { buildRegistry } from '../../src/components/registry'

const REGISTRY_PATH = path.join(process.cwd(), 'deploy/dist/registry')
...
//     const resolveFiles = item.files.map((file) => `../../src/components/${style.name}/${file}`)
...
// // Write style index.
// rimraf.sync(path.join(process.cwd(), 'deploy/dist/__registry__/index.ts'))
// fs.writeFileSync(path.join(process.cwd(), 'deploy/dist/__registry__/index.ts'), index)
  ...
  for (const item of result.data) {
    ...
    const files = item.files?.map((file) => {
      let content = fs.readFileSync(
        path.join(process.cwd(), 'src/components', style.name, file),
        'utf8'
      )
    ...
    fs.writeFileSync(path.join(targetPath, `${item.name}.json`), payloadStr, 'utf8')
  }
}
...
if (!fs.existsSync(colorsTargetPath)) fs.mkdirSync(colorsTargetPath, { recursive: true })
  ...
  for (const [mode, values] of Object.entries(colorMapping)) {
    ...
    for (const [key, value] of Object.entries(values)) {
      if (typeof value === 'string') {
        ...
        if (color) base.cssVars[mode][key] = color.hslChannel
      }
    }
  }
