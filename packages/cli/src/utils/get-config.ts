...
export const DEFAULT_COMPONENTS = '@/components/@materials'
export const DEFAULT_UTILS = '@/services/tw/utils'
...
export async function resolveConfigPaths(cwd: string, config: RawConfig) {
  ...
  if (config.typescript) {
    ...
    if ('paths' in tsConfig && Object.keys(tsConfig.paths).length === 0) {
      ...
      if (existsSync(tsConfigPath)) tsConfig = loadConfig(tsConfigPath)
    }
  } else {
...
export async function getRawConfig(cwd: string): Promise<RawConfig | null> {
  try {
    ...
    throw new Error(`Invalid configuration found in ${cwd}/.keen.json.`)
  }
}
